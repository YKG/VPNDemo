...
#ifdef AS_CLIENT
  snprintf(cmd, sizeof(cmd), "ifconfig tun0 10.9.0.2/16 mtu %d up", MTU);
#else
  snprintf(cmd, sizeof(cmd), "ifconfig tun0 10.9.0.1/16 mtu %d up", MTU);
#endif
  run(cmd);
}
...
#else
  run("iptables -t nat -A POSTROUTING -s 10.9.0.0/16 ! -d 10.9.0.0/16 -m comment --comment 'vpndemo' -j MASQUERADE");
  run("iptables -A FORWARD -s 10.9.0.0/16 -m state --state RELATED,ESTABLISHED -j ACCEPT");
  run("iptables -A FORWARD -d 10.9.0.0/16 -j ACCEPT");
#endif
}
...
#else
  run("iptables -t nat -D POSTROUTING -s 10.9.0.0/16 ! -d 10.9.0.0/16 -m comment --comment 'vpndemo' -j MASQUERADE");
  run("iptables -D FORWARD -s 10.9.0.0/16 -m state --state RELATED,ESTABLISHED -j ACCEPT");
  run("iptables -D FORWARD -d 10.9.0.0/16 -j ACCEPT");
#endif
}
...
int udp_bind(struct sockaddr *addr, socklen_t* addrlen) {
  ...
  *addrlen = result->ai_addrlen;
  ...
  if (flags != -1) {
    //if (-1 != fcntl(sock, F_SETFL, flags | O_NONBLOCK))//errno 11 sendto resource tmp unavaliable
